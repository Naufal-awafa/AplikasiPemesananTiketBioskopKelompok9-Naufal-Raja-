...
class Database
{
    ...
    private function buatTabelFitur(): void
    {
        $this->koneksi->exec(
            ...
            CREATE TABLE IF NOT EXISTS transaksi_produk (id INT AUTO_INCREMENT PRIMARY KEY,kode_transaksi VARCHAR(40) NOT NULL UNIQUE,kasir_id INT NOT NULL,tiket_id INT DEFAULT NULL,nama_pelanggan VARCHAR(150) DEFAULT '',total_harga INT NOT NULL,metode_bayar VARCHAR(100) NOT NULL,status VARCHAR(30) NOT NULL DEFAULT 'sukses',dibuat_pada DATETIME DEFAULT CURRENT_TIMESTAMP,INDEX idx_transaksi_produk_kasir(kasir_id,dibuat_pada),INDEX idx_transaksi_produk_tiket(tiket_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
            CREATE TABLE IF NOT EXISTS detail_transaksi_produk (id INT AUTO_INCREMENT PRIMARY KEY,transaksi_produk_id INT NOT NULL,produk_id INT NOT NULL,jumlah INT NOT NULL,harga_satuan INT NOT NULL,ukuran VARCHAR(20) NOT NULL DEFAULT 'regular',INDEX idx_detail_transaksi_produk(transaksi_produk_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
